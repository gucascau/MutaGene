...
print OUT "time\tstatus\tx\tMutationload\tSamples\n";
foreach my $i (keys %hash){
	if (exists $mutant{$i}){
		print OUT "$time_s{$i}\t$status_s{$i}\tMutant\t$ml{$i}\t$i\n";
		push @mut,$i;
	}else{
		print OUT"$time_s{$i}\t$status_s{$i}\tNonMutant\t$ml{$i}\t$i\n";
		push @nonmut,$i;
	}
}
